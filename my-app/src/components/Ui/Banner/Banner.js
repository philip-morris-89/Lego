...
export function Banner({
 ...
 const BANNERCOLOR = BANNERCOLORS.includes(bannerColor) ? bannerColor : BANNERCOLORS[0]
 const POSITION = POSITIONS.includes(position) ? position : POSITIONS[0]
 
 
    return(
       
        <div className="banner" data-size={SIZE} data-color={COLOR} data-background={BANNERCOLOR} data-position={POSITION} data-skeleton={skeleton}>
        <section className="bannerMedia">
          <picture className="bannerPicture">
            <source className="bannerPicture-source" type="image/webp" media="(min-width: 1024px)" srcset={imgDesktop}/>
            <source className="bannerPicture-source" type="image/webp" media="(min-width: 425px)" srcset={imgTablet}/>
